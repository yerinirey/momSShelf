...
const PATTERNS = ["plain", "stripes", "dots", "diamond", "wave"];
...
Deno.serve(async (req: Request) => {
  ...
  if (!authHeader) return jsonError(401, "Authorization 헤더 없음");
  ...
  if (!user?.email || !ALLOWED_EMAILS.includes(user.email)) {
    return jsonError(403, "권한 없음");
  }

  // body 검증
  let body: { title?: string; type?: string; author?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (!title || (type !== "novel" && type !== "movie")) {
    return jsonError(400, "title과 type 필수");
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        send({ type: "started", estimated_chars: ESTIMATED_OUTPUT_CHARS });
